			...
			<div class="row mt-5">
				...
				<div class="col-12 col-lg-10 mb-5">
					...
					<div class="border-bottom border-dark align-items-center pb-3">
						...
						@isset($famousPosts)
							<div class="mt-5 border-bottom border-dark align-items-center">
								<h2 class="pb-3 mb-0 mt-1 mr-auto">人気の記事 top10</h2>
								<!-- <a id="delete_selected" href="#" class="">選択削除</a> -->
							</div>
							@foreach($famousPosts as $famousPost)
								<div class="post py-1 d-flex flex-row align-items-center border-bottom border-dark">
									<a class="d-block rounded post_link pl-2 ml-1 mr-1 w-100 py-2" href="{{ action('FirstController@show', $famousPost) }}">{{ $famousPost->title }}</a>


									@isset($name_flag)
									<div class="pr-3 text-nowrap">{{ $famousPost->user->name }} さん</div>
									@endisset
									<div class="text-nowrap mr-3"><span class="font-weight-bold mr-1">{{ $famousPost->view_count }}</span>views</div>

									<div class="pr-1 text-nowrap">{{ $famousPost->created_at->format('Y/m/d') }}</div>
								</div>
							@endforeach
						@endisset
